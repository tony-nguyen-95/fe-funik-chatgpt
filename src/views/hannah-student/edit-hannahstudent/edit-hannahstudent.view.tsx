...
import { IEditHannahStudent } from './edit-hannahstudent.type';

import { IoMdCloseCircle } from 'react-icons/io';

const prefixClassName = 'edit-student';

export const EditHannahStudent: React.FC<IEditHannahStudent> = (props) => {
  ...
  useEffect(() => {
    if (baseInfo) {
      ...
      progress.value = baseInfo.progress.toString();
    }
  }, [baseInfo]);
  ...
  return (
    <div
      className={`fixed h-screen w-screen z-[100] top-1/2 left-1/2 
			...
			`}
    >
      <div className="bg-white w-[95%] md:w-auto md:min-w-[40rem] min-h-[28rem] py-5 flex flex-col justify-center items-center rounded-lg relative animate-popup">
        <div
        ...
        <form method="POST" onSubmit={callback} className="w-full min-h-[25rem] py-2 px-4 md:px-8 mt-12">
          ...
          <label className="text-md font-semibold mt-3">Tiến độ</label>
          <br />
          <select className="w-full p-2 border border-gray-300 rounded-md" id="progress" name="progress">
            <option value="0">Không xác định</option>
            <option value="1">Trễ tiến độ</option>
            <option value="2">Đúng tiến độ</option>
